...
void NoC::buildCommon() // FM: Common to every topology, called by the build_TopologyName functions
{
//    cout << "Inside buildCommon function" << endl;
    ...
    // FM: The followings are looking for the user-defined routing and traffic
...
void NoC::buildMesh()
{
    cout << "Building mesh topology"<<endl;
    buildCommon();
    ...
    for (int i = 0; i < GlobalParams::mesh_dim_x; i++) { // FM: Fixing the column, running on the row
        // Create the single Tile with a proper name
        char tile_name[64];
        Coord tile_coord;
        tile_coord.x = i;
        ...
        // Tell to the router its coordinates
        t[i][j]->r->configure(j * GlobalParams::mesh_dim_x + i, // FM: Tile->Router->configure (Need to open the ROuther.cpp to know this)
                  GlobalParams::stats_warm_up_time,
                  GlobalParams::buffer_depth,
                  grtable);
        ...
        // Tell to the PE its coordinates
        t[i][j]->pe->local_id = j * GlobalParams::mesh_dim_x + i; // Same comments as for the Router
    ...
    // FM: this may be modified in order to make the "folded" connections
    for (int i = 0; i <= GlobalParams::mesh_dim_x; i++) {
